# Current visual set — 2026-09-24

`art/cosmic/` contains 51 original, editable SVG assets for the rc3 visual overhaul. Character silhouettes use plum outlines, cream faces, orange/berry accents and supporting teal/lavender colours. `tools/make_vector_art.py` generates the 38 principal character/icon assets; crosshair, spark, debris and vortex SVGs are standalone sources. No third-party images or raster generation were used for this set.

Animation and integration: `PlanetVisual.cs` (face/blink/recoil), `Body.cs` (enemy motion), `Boss.cs` and subclasses (breathing/attack anticipation), `Bullet.cs`, `Moon.cs`, `PowerUp.cs`, and `CosmicBackdrop.cs`. `ArcadeSkin.cs` defines shared UI styling. The menu uses the same illustrated art. `tools/refine_cosmic_art.py` runs after the base generator to rebuild faces, planet surfaces, ring halves, shield, eclipse, armour, round shots and badges. The old menu preservation hash is historical; the user explicitly requested its redesign.

The older inventory below is historical and is superseded by `VISUAL_DIRECTION.md` and `RELEASE.md`.

# Current release assets (2026-09-23)

The inventory below is historical planning, not a release checklist. The current single-mode build preserves the smiling planets and space silhouettes, uses Lilita One under SIL OFL (fonts/OFL-LilitaOne.txt), and uses original synthesized music and effects generated by tools/make_audio.py. Previous audio and Bubblegum.ttf are excluded from release exports. See RELEASE.md for the shipped scope.
...
Bodies — new kinds need **distinct silhouettes**, not just recolours. **Done**,
and taken further than originally scoped here: every `BodyKind` now has its own
silhouette, not just the four that were flagged below, so a busy fight reads
correctly even for the three kinds that used to share a plain face. Masters are
flat-vector SVG in `art_source/`, exported per the sizes in section 2. Wired up
in `Body.cs` (one texture per `BodyKind`, replacing the old shared
`enemy N.png` pool and the `TextureIndex` indirection) and `BodyMark.cs` (the
Satellite stand-in fins are gone now that fins are baked into the sprite; the
Bulwark plate, Flare fuse and Fracture/Splinter seams stay, since those add a
colour or motion the flat-tinted sprite can't).
- [x] `body_fracture.png` — visibly segmented, reads as "will break apart"
- [x] `body_satellite.png` — directional fin so its facing is legible
- [x] `body_flare.png` — bloated, lit crack pattern
- [x] `body_bulwark.png` — plated front arc, obviously flankable
- [ ] `body_pulsar.png` — single big eye, long thin body. **Not a `BodyKind`** —
	  no enemy of this name exists in `BodyKind.cs`; leaving unstarted rather
	  than guessing what it was meant for. Possibly meant for a future Pulsar
	  boss ability (see `telegraph_line.png` / `pulsar_charge.ogg` below), not
	  the bestiary — check before drawing it.
- [x] `body_fracture_mini.png` — 128×112, the split children
- [x] `body_drifter.png`, `body_shard.png`, `body_planetoid.png` — not
	  originally listed here (tint/scale were judged enough), but the baseline,
	  swarmer and tank read as three sizes of the same blank face in a crowded
	  fight. Drawn to match: plain circle, small angular shard, big cratered
	  sphere.
...
| M3 | ~23 | 11 | Bestiary silhouettes done (8, incl. 3 bonus); weapons and Coil remain |
...
6. ~~**Bestiary silhouettes** (8 sprites).~~ Done out of order, ahead of the
   weapon split above — every `BodyKind` now has a distinct shape, not just a
   tint. See M3 above.
7. Then follow the milestones in order.
